...
typedef struct AxiomUsbCmdHeader_struct
{
    u8 usbReportId;
    u8 AxiomBridgeCmd;
    u8 wrLen;
    u8 rdLen;
    AxiomCmdHeader i2c;
} AxiomUsbCmdHeader, *pAxiomUsbCmdHeader;
...
int get_rx_chunks(int usage_table_bytes)
{
    ...
    if ((((usage_table_bytes % RX_PAYLOAD_MAX) != 0) &&
         (usage_table_bytes > RX_PAYLOAD_MAX)) ||
        (usage_table_bytes < RX_PAYLOAD_MAX))
        return ret + 1;
    else
        return ret;
...
int axiom_usb_do_comms(struct axiom_data *data)
{
    ...
    pAxiomUsbCmdHeader usbCmd = (pAxiomUsbCmdHeader)&data->tx_buf[0];
    int timeout = AX_USB_TIMEOUT;
    int ret = 0;

    hid_dbg(hdev, "AXIOM: axiom_usb_do_comms...CMD=0x%02x\n", tx_buf[1]);
    ...
    if (usbCmd->AxiomBridgeCmd == AX_TBP_I2C_DEVICE1)
    {
        if (usbCmd->i2c.read == 0 && usbCmd->i2c.length > 0) 
            hid_dbg(hdev, "i2c write: %*ph\n", usbCmd->i2c.length, usbCmd->i2c.writeData);
    }
    ...
    if (ret > 0)
    {
        hid_dbg(hdev, "AXIOM:...axiom_usb_do_comms RESP=0x%02x rc=%d\n", rx_buf[0], ret);
        hid_dbg(hdev, "HID response header: %*ph\n", 2, &rx_buf[0]);
        if (rx_buf[1] > 0) hid_dbg(hdev, "Axiom data: %*ph\n", rx_buf[1], &rx_buf[2]);
    } 
    else
    {
        hid_err(hdev, "ERROR:...axiom_usb_do_comms rc=%d\n", ret);
    }
    return ret;
}
...
static u16 axiom_read_usage(void *pAxiomData, u8 usage, u8 page, u16 length, u8 *pBuffer)
{
    ...
    hid_info(hdev,"axiom_read_usage(usage 0x%02x, page %u, length %u)", usage,  page, length);
    ...
    for (chunk = 0; chunk < numberOfBlocks; chunk++)
    {
        ...
        usbCmd->usbReportId = 0;
        usbCmd->AxiomBridgeCmd = AX_TBP_I2C_DEVICE1;
        usbCmd->wrLen = sizeof(AxiomCmdHeader);
        usbCmd->rdLen = bytes_to_read;
        usbCmd->i2c.target_address = usage_to_target_address(&data->data_core, usage, chunkPage, bytes_read);
        usbCmd->i2c.length = bytes_to_read;
        usbCmd->i2c.read = 1;
        spin_unlock_irqrestore(lock, *flags);
        ...
        spin_unlock_irqrestore(lock, *flags);
        hid_info(hdev, "axiom_read_usage: bytes read: %d, chunk %d\n",
                bytes_read, chunk);
    }
...
static u16 axiom_write_usage(void *pAxiomData, u8 usage, u8 page, u16 length, u8 *pBuffer)
{
    ...
    usbCmd->wrLen = sizeof(AxiomCmdHeader);
    usbCmd->rdLen = length;
    usbCmd->i2c.target_address = usage_to_target_address(&data->data_core, usage, page, 0);
    usbCmd->i2c.length = length;
    usbCmd->i2c.read = 0;
    memcpy(usbCmd->i2c.writeData, pBuffer, length);
    spin_unlock_irqrestore(lock, *flags);
...
int axiom_stop_proxy(struct hid_device *hdev, struct axiom_data *data)
{
    ...
    hid_info(hdev, "AXIOM: axiom_stop_proxy\n");
    ...
    hid_info(hdev, "AXIOM:...axiom_stop_proxy rc=%d\n", ret);
    return ret;
}
...
static void axiom_usb_setup(struct work_struct *work)
{
    ...
    if (0 == strncmp("TouchNetix TNxPB-005", hdev->name, 20)) 
    {
        data->axiom_hid_payload_size = HID_PAYLOAD_SIZE_TNxPB005;
    }
    else if (0 == strncmp("TouchNetix TNxPB-007", hdev->name, 20)) 
    {
        data->axiom_hid_payload_size = HID_PAYLOAD_SIZE_TNxPB007;
    }
    else if (0 == strncmp("TouchNetix AXPB009", hdev->name, 20)) 
    {
                           ...
                           const struct hid_device_id *id)
{
    ...
    {
        hid_info(hdev, "AXIOM: axiom_usb_probe interface %d, done\n", iface_no);
...
static void axiom_usb_remove(struct hid_device *hdev)
{
    ...
    if (iface_no != AX_IF_TBPCTRL)
    {
        hid_info(hdev, "AXIOM: axiom_usb_remove interface %d removed\n", iface_no);
        return;
    }
                               ...
                               int size)
{
    ...
    if (iface_no != AX_IF_TBPCTRL)
    {
        hid_info(hdev, "AXIOM: axiom_usb_raw_event on interface %d not handled!\n", iface_no);
        return 0;
    }
